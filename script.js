document.addEventListener('DOMContentLoaded', () => {
    ...
    const confirmTreats = document.getElementById('confirm-treats');
    ...
    /* ---------- treats + day → final letter ---------- */
    treatNextBtn.addEventListener('click', () => {
        chosenDay = daySelect.value;
        const treats = Array.from(document.querySelectorAll('input[name="treat"]:checked')).map(cb => cb.value);
        ...
        if (treats.length < 1) {
            setMood('question');
            alert("Pick at least one treat — it's literally free for you.");
            return;
        }
        if (treats.length > 3) {
            setMood('question');
            alert('Whoa, max 3 — I said sorry, not that I got a raise. 😭');
            return;
        }
        ...
        confirmTreats.textContent = treats.join(', ');
        confettiLeaves();
        sendEmail(chosenDay, treats);
    });
